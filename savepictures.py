...
NUM_CLASSES = 19
NUM_STEPS = 62500
NUM_STEPS_STOP = 70000 # early stopping
POWER = 0.9
RANDOM_SEED = 1234
...
def main_worker(gpu, world_size, dist_url):
    ...
    if args.model == 'DeepLab':
        if args.resume:
            ...
            model_B2_weights = resume_weight['model_I1_state_dict']
            model_B_weights = resume_weight['model_I2_state_dict']
            head_weights = resume_weight['Ihead_state_dict']
            classifier_weights = resume_weight['Iclassifier_state_dict']
...
def validate(model_B2, model_B, head, classifier, seg_loss, gpu, logger, testloader, output_folder):
    ...
    with torch.no_grad():
        for i, batch in enumerate(testloader):
            ...
            labels[labels==9]=args.ignore_label
            labels[labels==14]=args.ignore_label
            labels[labels==16]=args.ignore_label
            for b in range(output_np.shape[0]):
                output_b = output.cpu()[b,:,:].numpy()
                labels_b = labels.cpu()[b,:,:].numpy()
                intersection_b, union_b, _ = intersectionAndUnion(output_b, labels_b, args.num_classes, args.ignore_label)
                iou_b = intersection_b.sum() / (union_b.sum() + 1e-10)
                iou_b = "{:.2f}".format(iou_b)
                mask_filename = img_paths[b].split("/")[-1].split(".")[0]
                filename = os.path.join(output_folder, mask_filename+iou_b+".png")
                img_color = colorize_mask(output.cpu()[b,:,:].numpy())
                img_color.save(filename)
