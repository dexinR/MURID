...
def get_arguments():
    ...
    parser.add_argument("--lambda_distillation_start", type=float, default=LAMBDA_DISTILLATION, help='start weight for loss_distillation')
...
def adjust_lambda_Distillation_loss(lambda_Distillation, i_iter, max_iter):
    if i_iter>3/4*max_iter:
        lambda_Distillation=10
    else:
        if i_iter>1/3*max_iter:
            lambda_Distillation=1
    return lambda_Distillation

def main_worker(gpu, args, world_size=1, dist_url=None):
    ...
    if args.model == 'DeepLab':
        ...
        feat_dim = ndf
        model_MAB1 = MAB(args.n_heads,feat_dim,feat_dim,feat_dim, ln=False)
        model_MAB2 = MAB(args.n_heads,feat_dim,feat_dim,feat_dim, ln=False)
        ...
        if args.layer == 0:
            ndf = 64
            model_I1 = nn.Sequential(model.backbone.conv1, model.backbone.bn1, model.backbone.relu, model.backbone.maxpool)
            model_I2 = nn.Sequential(model.backbone.layer1, model.backbone.layer2, model.backbone.layer3, model.backbone.layer4)
        elif args.layer == 1:
            ndf = 256
            model_I1 = nn.Sequential(model.backbone.conv1, model.backbone.bn1, model.backbone.relu, model.backbone.maxpool, model.backbone.layer1)
            model_I2 = nn.Sequential(model.backbone.layer2, model.backbone.layer3, model.backbone.layer4)
        elif args.layer == 2:
            ndf = 512
            model_I1 = nn.Sequential(model.backbone.conv1, model.backbone.bn1, model.backbone.relu, model.backbone.maxpool, model.backbone.layer1, model.backbone.layer2)
            model_I2= nn.Sequential(model.backbone.layer3, model.backbone.layer4)
        Iclassifier = ASPP_Classifier_Gen(2048, [6, 12, 18, 24], [6, 12, 18, 24], args.num_classes, hidden_dim=args.hidden_dim)
        Ihead, Iclassifier = Iclassifier.head, Iclassifier.classifier
    ...
    if gpu == 0:
        ...
        logger.info(model_MAB1)
        logger.info(model_MAB2)
        logger.info(model_B)
        ...
        logger.info(model_I1)
        logger.info(model_I2)
        logger.info(Ihead)
        logger.info(Iclassifier)
    else:
        logger = None
    ...
    model_params = [{'params': list(model_B1.parameters()) + list(model_B2.parameters()) + list(model_MAB1.parameters()) +list(model_MAB2.parameters()) + list(model_B.parameters()) + list(model_I1.parameters()) + list(model_I2.parameters())},
                    {'params': list(head.parameters()) + list(classifier.parameters()) + list(Ihead.parameters()) + list(Iclassifier.parameters()) + \
    ...
    if args.dist:
        model_B1 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_B1).cuda()
        model_B1 = torch.nn.parallel.DistributedDataParallel(model_B1, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_B1 = model_B1.cuda()

    if args.dist:
        model_B2 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_B2).cuda()
    ...
    if args.dist:
        model_MAB1 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_MAB1).cuda()
        model_MAB1 = torch.nn.parallel.DistributedDataParallel(model_MAB1, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_MAB1 = model_MAB1.cuda()

    if args.dist:
        model_MAB2 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_MAB2).cuda()
        model_MAB2 = torch.nn.parallel.DistributedDataParallel(model_MAB2, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_MAB2 = model_MAB2.cuda()

    if args.dist:
        model_B = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_B).cuda()
        model_B = torch.nn.parallel.DistributedDataParallel(model_B, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_B = model_B.cuda()

    if args.dist:
        head = torch.nn.SyncBatchNorm.convert_sync_batchnorm(head).cuda()
        head = torch.nn.parallel.DistributedDataParallel(head, device_ids=[gpu], find_unused_parameters=True)
    else: 
        head = head.cuda()

    if args.dist:
        classifier = torch.nn.SyncBatchNorm.convert_sync_batchnorm(classifier).cuda()
        classifier = torch.nn.parallel.DistributedDataParallel(classifier, device_ids=[gpu], find_unused_parameters=True)
    else: 
        classifier = classifier.cuda()

    if args.dist:
        model_D1 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_D1).cuda()
        model_D1 = torch.nn.parallel.DistributedDataParallel(model_D1, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_D1 = model_D1.cuda()

    if args.dist:
        model_D2 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_D2).cuda()
        model_D2 = torch.nn.parallel.DistributedDataParallel(model_D2, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_D2 = model_D2.cuda()

    if args.dist:
        aux_classifier = torch.nn.SyncBatchNorm.convert_sync_batchnorm(aux_classifier).cuda()
        aux_classifier = torch.nn.parallel.DistributedDataParallel(aux_classifier, device_ids=[gpu], find_unused_parameters=True)
    else: 
        aux_classifier = aux_classifier.cuda()

    if args.dist:
        model_I1 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_I1).cuda()
        model_I1 = torch.nn.parallel.DistributedDataParallel(model_I1, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_I1 = model_I1.cuda()

    if args.dist:
        model_I2 = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model_I2).cuda()
        model_I2 = torch.nn.parallel.DistributedDataParallel(model_I2, device_ids=[gpu], find_unused_parameters=True)
    else: 
        model_I2 = model_I2.cuda()

    if args.dist:
        Ihead = torch.nn.SyncBatchNorm.convert_sync_batchnorm(Ihead).cuda()
        Ihead = torch.nn.parallel.DistributedDataParallel(Ihead, device_ids=[gpu], find_unused_parameters=True)
    else: 
        Ihead = Ihead.cuda()

    if args.dist:
        Iclassifier = torch.nn.SyncBatchNorm.convert_sync_batchnorm(Iclassifier).cuda()
        Iclassifier = torch.nn.parallel.DistributedDataParallel(Iclassifier, device_ids=[gpu], find_unused_parameters=True)
    else: 
        Iclassifier = Iclassifier.cuda()
    ...
    interp_distillation = nn.Upsample(size=(args.rcrop[1], args.rcrop[0]), mode='bilinear', align_corners=True)
    ...
    for i_iter in range(args.num_steps):
        ...
        lambda_Distillation = adjust_lambda_Distillation_loss(args.lambda_distillation_start, i_iter, args.num_steps)
        ...
        for sub_i in range(args.iter_size):
            ...
            try:
                _, batch_s = trainloader_iter.__next__()
            ...
            images_s = batch_s['img'].cuda()
            labels_s = batch_s['label'].cuda()   
            images_t = batch_t['img'].cuda()
            
            src_size = images_s.shape[-2:]
            tgt_size = images_t.shape[-2:]
            
            with torch.cuda.amp.autocast():
                feat_src = model_B1(images_s)
                feat_src_size = feat_src.size()
                feat_tgt = model_B2(images_t) 
                feat_tgt_size = feat_tgt.size()
                
            
                
                feat_src_MAB , _ = model_MAB1(feat_tgt.view(args.batch_size,-1, ndf),feat_src.view(args.batch_size,-1, ndf),True)
                feat_tgt_MAB , _ = model_MAB2(feat_src.view(args.batch_size,-1, ndf),feat_tgt.view(args.batch_size,-1, ndf),True)
                
                feat_src_MAB = feat_src_MAB.view(feat_src_size)
                feat_tgt_MAB = feat_tgt_MAB.view(feat_tgt_size)
                
                feat_src = torch.add(feat_src, feat_src_MAB)
            
                
                feat_B_src = model_B(feat_src) 
      
                feat_B_src_head = head(feat_B_src) 
                pred_src = classifier(feat_B_src_head) 
                pred_src = interp(pred_src)

                temperature = 1.8
                pred_src = pred_src.div(temperature)
               
                loss_seg = seg_loss(pred_src, labels_s) 
                ...
                loss_s = loss_seg + loss_adv_src

             
                loss_s = loss_s / args.iter_size
            ...
            with torch.cuda.amp.autocast():
                ...
                feat_src_MAB = feat_src_MAB.view(feat_src_size)
                feat_tgt_MAB = feat_tgt_MAB.view(feat_tgt_size)
                feat_tgt = torch.add(feat_tgt, feat_tgt_MAB)
                ...
                with torch.no_grad():
                    pred_logits, pred_idx = F.softmax(pred_tgt.detach(), 1).max(1)
                    ...
                    if i_iter % 500 == 0 and gpu == 0:
                        for c in range(args.num_classes):
                            ...
                            logger.info("c: {}, class_i: {} threshold: {}, len(conf_bank[c]): {}".format(c, class_list[c], thresholds[c], len(conf_bank[c])))
                ...
                loss_t = loss_adv_tgt + loss_st

                loss_t = loss_t / args.iter_size
            ...
            with torch.cuda.amp.autocast():
                ...
                pred_tgt = interp_target(pred_tgt)
                pred_tgt = pred_tgt.div(temperature)
                pred_tgt_Softmax = F.softmax(pred_tgt, dim = 1)
                pred_Distillation = model_I2(model_I1(images_t))
                pred_Distillation = Iclassifier(Ihead(pred_Distillation))
                pred_Distillation = interp_distillation(pred_Distillation)
                pred_Distillation = pred_Distillation.div(temperature)
                pred_Distillation_Softmax = F.softmax(pred_Distillation, dim = 1)
      
                loss_Distillation = lambda_Distillation * args.num_classes * F.kl_div(pred_Distillation_Softmax.log(), pred_tgt_Softmax.detach(), reduction='mean')
            
                loss_Distillation_value += loss_Distillation / args.iter_size
            scaler.scale(loss_Distillation).backward()    
            ...
            with torch.cuda.amp.autocast():
                src_D2_pred = model_D2(F.softmax(pred_src.detach(), 1))
        ...
        loss_Distillation_value = loss_Distillation_value.item()/ n.item()
        ...
        if gpu == 0 and i_iter % args.print_every == 0:
            logger.info('iter = {0:8d}/{1:8d}, loss_seg = {2:.3f}, loss_adv_src = {3:.5f}, loss_adv_tgt = {4:.5f}, loss_D1 = {5:.3f}, '
                'loss_D2 = {6:.3f}, loss_st = {7:.5f}, loss_Distillation={8:.5f}, epoch_s = {9:3d}, epoch_t = {10:3d}'.format(i_iter, args.num_steps, loss_seg_value, loss_adv_src_value, \
                loss_adv_tgt_value, loss_D1_value, loss_D2_value, loss_st_value, loss_Distillation_value, epoch_s, epoch_t))
        
        if gpu == 0 and i_iter == args.num_steps_stop - 1:
            logger.info('save model ...')
            filename = osp.join(args.snapshot_dir, 'GTA5_' + str(args.num_steps_stop) + '.pth')
            save_file = {'model_B1_state_dict': model_B1.state_dict(), 'model_B2_state_dict': model_B2.state_dict(),'model_MAB1_state_dict': model_MAB1.state_dict(),\
                    'model_MAB2_state_dict': model_MAB2.state_dict(),'model_B_state_dict': model_B.state_dict(), 'head_state_dict': head.state_dict(), \
                    'classifier_state_dict': classifier.state_dict(),'model_I1_state_dict': model_I1.state_dict(),'model_I2_state_dict': model_I2.state_dict(),\
                    'Ihead_state_dict': Ihead.state_dict(),'Iclassifier_state_dict': Iclassifier.state_dict()\
                    'model_D1_state_dict': model_D1.state_dict(),'model_D2_state_dict': model_D2.state_dict(),'aux_classifier_state_dict': aux_classifier.state_dict()}
            torch.save(save_file, filename)
            logger.info("saving checkpoint model to {}".format(filename))
           
        
        if i_iter % args.save_pred_every == 0 and i_iter != 0:
            miou, loss_val = validate(model_I1, model_I2, Ihead, Iclassifier, seg_loss, gpu, logger if gpu == 0 else None, datasets.target_valid_loader,i_iter)
            if args.tensorboard and gpu == 0:
                scalar_info = {
                    'miou_val': miou,
                    'loss_val': loss_val
...
def validate(model_I1, model_I2, Ihead, Iclassifier, seg_loss, gpu, logger, testloader, i_iter):
    if gpu == 0:
    ...
    with torch.no_grad():
        for i, batch in enumerate(testloader):
            ...
            pred = model_I2(model_I1(images))
            pred = Iclassifier(Ihead(pred))
...
if __name__ == '__main__':
    ...
    if args.dist:
        ...
        mp.spawn(main_worker, nprocs=args.world_size, args=(args, args.world_size, args.dist_url))
    else:
