...
class CustomDatasetDataLoader():
    def __init__(self, opt, logger):
        ...
        ## create train loader
        self.source_train_sampler = torch.utils.data.distributed.DistributedSampler(self.source_train, shuffle=not opt.noshuffle) if opt.dist else None
        self.source_train_loader = torch.utils.data.DataLoader(
            self.source_train,
            batch_size=opt.batch_size,
            shuffle=False,
            sampler=self.source_train_sampler,
            num_workers=int(opt.num_workers),
            drop_last=True,
            pin_memory=True,
        )
        self.target_train_sampler = torch.utils.data.distributed.DistributedSampler(self.target_train, shuffle=not opt.noshuffle) if opt.dist else None
        self.target_train_loader = torch.utils.data.DataLoader(
            self.target_train,
            batch_size=opt.batch_size,
            shuffle=False,
            sampler=self.target_train_sampler,
            num_workers=int(opt.num_workers),
        ...
        target_valid = find_dataset_using_name(opt.tgt_val_dataset)
        ...
        self.target_valid_sampler = torch.utils.data.distributed.DistributedSampler(self.target_valid, shuffle=False) if opt.dist else None
        self.target_valid_loader = torch.utils.data.DataLoader(
            self.target_valid,
            batch_size=opt.batch_size_val,
            shuffle=False,
            sampler=self.target_valid_sampler,
            num_workers=int(opt.num_workers),
            drop_last=False,
            pin_memory=True,
        )
